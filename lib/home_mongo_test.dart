...
class HomeMongoTest extends StatefulWidget {
  HomeMongoTest({
    Key? key,
    required this.url,
  ...
  final String url;
  @override
  State<HomeMongoTest> createState() => _HomeMongoTestState();
}

class _HomeMongoTestState extends State<HomeMongoTest> {
  IO.Socket? socket;
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasData) {
                  ...
                  final List<int> numbers = data[0]['number']
                      .map<int>(
                          (number) => int.tryParse(number.toString()) ?? 0)
                      .toList();
                  print('list number home mongo test: ${numbers.length}');
